...
class YodelDateTimePickerModel extends CommonPickerModel {
  final DateTime maxTime;
  final DateTime minTime;
  final int minuteInterval;
  List<DateTime> dateList = [];

  YodelDateTimePickerModel({
    ...
    this.currentTime = DateTimeHelper.convertToIntervalMinutesDateTime(
        currentTime, minuteInterval);

    setLeftIndex(_getCurrentLeftIndex());
    setMiddleIndex(this.currentTime.hour);
    final minutesIndex = DateTimeHelper.getNearestIntervalTime(
        this.currentTime.minute, minuteInterval);
    setRightIndex(minutesIndex);
  }

  int _getCurrentLeftIndex() {
    final date = DateTimeHelper.toDateOnly(currentTime);
  ...
  void _fillLeftLists() {
    this.leftList =
        List.generate(maxTime.difference(minTime).inDays + 1, (int index) {
      DateTime time = minTime.add(Duration(days: index));
      dateList.add(DateTimeHelper.toDateOnly(time));
  ...
  void _fillRightLists() {
    final length =
        DateTimeHelper.getTimeToIntervalLength(interval: minuteInterval);
    this.rightList = List.generate(length, (int index) {
      return digits(minuteInterval * index, 2);
    });
  }
  ...
  @override
  DateTime finalTime() {
    DateTime time = minTime.add(Duration(days: currentLeftIndex()));
    return currentTime.isUtc
        ...
        : DateTime(time.year, time.month, time.day, currentMiddleIndex(),
            currentRightIndex() * minuteInterval);
  }
